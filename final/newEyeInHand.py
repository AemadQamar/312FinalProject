...
async def track_loop(ballColor):
    global latest_ball
    global camera, cx, cy, half_size

    circles = None

    while camera.IsGrabbing():
        grab_result = camera.RetrieveResult(5000, pylon.TimeoutHandling_ThrowException)
        frame_resized = cv2.resize(grab_result.Array[:, :, ::-1], (0,0), fx=0.5, fy=0.5)
        grab_result.Release()
